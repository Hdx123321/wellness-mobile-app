...
食物记录现在按 `BREAKFAST`、`LUNCH`、`DINNER`、`SNACK` 四种餐次保存。拍照分析本身仍使用 `POST /api/food/analyze`，但用户确认结果并调用 `POST /api/food/entries/analyzed` 时，请求体必须包含 `mealType`；普通目录选餐调用 `POST /api/food/entries` 时也相同。响应中的 Food entry 会返回 `mealType`。如调整拍照确认流程，必须保留餐次和用户所选记录日期，不能固定写入当天零食。
